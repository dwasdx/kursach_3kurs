...
final class LocationPickerViewController: UIViewController {
    ...
    lazy var locationButton: AlertableButton = {
...
//        $0.setImage(#imageLiteral(resourceName: "geolocation"), for: UIControl.State())
        $0.setImage(UIImage(systemName: "location.circle"), for: UIControl.State())
        $0.addTarget(self, action: #selector(LocationPickerViewController.currentLocationPressed),
                         for: .touchUpInside)
        return $0
    }(AlertableButton(frame: CGRect(x: 0, y: 0, width: 44, height: 44)))
